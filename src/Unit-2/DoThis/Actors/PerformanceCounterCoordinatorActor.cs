...
namespace ChartApp.Actors
{
    public class PerformanceCounterCoordinatorActor : ReceiveActor
    {
        ...
        public PerformanceCounterCoordinatorActor(IActorRef chartingActor, Dictionary<CounterType, IActorRef> counterActors)
        {
            ...
            Receive<Watch>(watch =>
            {
                if (!_counterActors.ContainsKey(watch.Counter))
                {
                    ...
                    var counterActor = Context.ActorOf(Props.Create(() => new PerformanceCounterActor(watch.Counter.ToString(), s_counterGenerators[watch.Counter])));
                ...
                _chartingActor.Tell(new ChartingActor.AddSeries(s_counterSeries[watch.Counter]()));
        ...
        /// we want to monitor
        /// </summary>
        private static readonly Dictionary<CounterType, Func<PerformanceCounter>> s_counterGenerators = new Dictionary<CounterType, Func<PerformanceCounter>>()
        {
        ...
        /// </summary>
        private static readonly Dictionary<CounterType, Func<Series>> s_counterSeries = new Dictionary<CounterType, Func<Series>>()
        {
